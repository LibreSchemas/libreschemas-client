...
const UnpopularBehaviourScreen = ({navigation, route}) => {
  ...
  const styles = StyleSheet.create({
    ...
    behaviourtype_heading: {
      fontSize: 16,
      alignSelf: 'center',
      color: colors.unpopularbehaviour,
      fontWeight: 'bold',
    }
  });
  ...
  function getData(docID) {
    ...
    const _goBack = () => { 
      ...
      const showEmptyListView = () => (
        <View style={styles.item}>
          <Text style={{color: colors.text}}>There are no Unpopular Behaviours to list here.</Text>
        </View>
      );
    ...
    const unsortedUnpopularBehaviours = behaviours.filter(function (behaviour) {
    ...
    const sortedBehavioursByRank = unsortedUnpopularBehaviours.sort((a, b) => (a.rank < b.rank ? -1 : 1))

    const appBarContentTitle = data.getSchema.name
    const appBarContentSubTitle = 'Unpopular Behaviours'
    ...
    const header = () => (
      <View style={styles.event}>
        ...
        <Text style={styles.behaviourtype_heading}>[ Unpopular Behaviour ]</Text>
        <View style={{ flexDirection: "row-reverse"}}>
          <Pressable onPress={() => {
                  if(Speech.isSpeakingAsync()) {
                    Speech.speak(`${event_name}. Unpopular Behaviour.`);
                  }
                }}>
                <MaterialCommunityIcons name="text-to-speech" color="#1ddbc9" size={22}/>
          </Pressable>
...
export default UnpopularBehaviourScreen;
