...
const UnlawfulBehaviourScreen = ({navigation, route  }) => {
  ...
  function getData(docID) {
    ...
    const unsortedUnlawfulBehaviours = behaviours.filter(function (behaviour) {
    ...
    const sortedBehavioursByRank = unsortedUnlawfulBehaviours.sort((a, b) => (a.rank < b.rank ? -1 : 1))
    ...
    const showEmptyListView = () => (
        <View style={styles.item}>
          <Text style={{color: colors.text}}>There are no Unlawful Behaviours to list here.</Text>
        </View>
    );

    const appBarContentTitle = data.getSchema.name
    const appBarContentSubTitle = 'Unlawful Behaviours'
    ...
    const header = () => (
      <View style={styles.event}>
        ...
        <Text style={styles.behaviourtype_heading}>[ Unlawful Behaviour ]</Text>
        <View style={{ flexDirection: "row-reverse"}}>
          <Pressable onPress={() => {
                  if(Speech.isSpeakingAsync()) {
                    Speech.speak(`${event_name}. Unlawful Behaviour.`);
                  }
                }}>
                <MaterialCommunityIcons name="text-to-speech" color="#1ddbc9" size={22}/>
          </Pressable>
...
export default UnlawfulBehaviourScreen;
